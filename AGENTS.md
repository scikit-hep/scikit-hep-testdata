...
`known_files` is loaded from `src/skhep_testdata/data/file_list.txt`, which is **generated by `setup.py` at build time** by scanning `data/` for `.root/.lhe/.gz/.json`. Don't hand-edit `file_list.txt`.
